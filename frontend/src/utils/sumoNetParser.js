...
export async function parseSumoNetXml(url) {
  ...
  if (typeof Worker !== 'undefined') {
    try {
      const worker = new Worker(new URL('../workers/sumoNetWorker.js', import.meta.url));
      const result = await new Promise((resolve, reject) => {
        ...
        worker.onmessage = (e) => {
          ...
          if (type === 'result') { clearTimeout(timer); resolve(data); try { worker.terminate(); } catch (_) {} }
          else if (type === 'error') { clearTimeout(timer); reject(new Error(message || 'Worker error')); try { worker.terminate(); } catch (_) {} }
        };
        worker.onerror = (err) => { clearTimeout(timer); reject(new Error(err.message || 'Worker failed')); try { worker.terminate(); } catch (_) {} };
  ...
  let res = await fetch(bustUrl, { cache: "no-store", credentials: "same-origin" });
  if (!res.ok) {
