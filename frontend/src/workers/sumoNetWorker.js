...
function simplifyRDP(points, epsilon) {
  if (!Array.isArray(points) || points.length <= 2 || !epsilon || epsilon <= 0) return points;
  const sq = (n) => n * n;
  ...
  while (stack.length) {
    ...
    for (let i = start + 1; i < end; i++) {
      ...
      if (d > maxDistSq) { maxDistSq = d; idx = i; }
    }
...
async function parseXmlInWorker(url) {
  ...
  const laneNodes = filteredEdges.flatMap((e) => Array.from(e.querySelectorAll('lane[shape]')));
