...
const TLSTestPanel = () => {
  ...
  const executeDirectStateCommand = async (phase) => {
    if (!canControl) {
      setLastResult({
        type: "error",
        message: "No permission to control traffic lights",
      });
      return;
    }
  ...
  if (!user) {
    return (
      <PageLayout
        ...
        subtitle="Traffic Light Control Testing"
      >
      ...
      <div className="tls-test-panel">
        ...
        <div className="tls-selector">
          ...
          {loadingTlsData ? (
            ...
            <select
              ...
              disabled={
                !tlsData ||
                !tlsData.friendlyNames ||
                tlsData.friendlyNames.length === 0
              }
            >
              {tlsData && tlsData.friendlyNames ? (
                tlsData.friendlyNames.map((friendlyName) => {
                  const actualId =
                    tlsData.mappings[friendlyName] || friendlyName;
                  return (
                    <option key={friendlyName} value={friendlyName}>
                      {friendlyName} (
                      {actualId.length > 30
                        ? actualId.substring(0, 30) + "..."
                        : actualId}
                      )
                    </option>
                  );
        ...
        {tlsData && !loadingTlsData && (
          <div className="tls-info">
            <p>
              📊 <strong>Available Traffic Lights:</strong>{" "}
              {tlsData.friendlyCount} friendly names mapped to{" "}
              {tlsData.totalCount} total TLS IDs
            </p>
            {selectedTls && tlsData.mappings[selectedTls] && (
              <p>
                🎯 <strong>Selected:</strong> "{selectedTls}" → "
                {tlsData.mappings[selectedTls]}"
              </p>
            )}
          </div>
        )}
        ...
        <div className="state-control-section">
          ...
          <p>
            Set exact traffic light state using SUMO phase notation (e.g.,
            "GrGr", "rGrG")
          </p>
          ...
          <div className="preset-buttons">
            <button
              ...
              disabled={
                loading || !canControl || !selectedTls || loadingTlsData
              }
        ...
        {!canControl && (
          <div className="permission-info">
            <p>
              🔒 Your role ({user.role}) does not allow traffic light control.
            </p>
        ...
        <div className="instructions">
          ...
          <ul>
            <li>
              Select a traffic light from the dropdown (friendly names mapped to
              real SUMO IDs)
            </li>
            <li>
              Ensure the SUMO simulation is running via the "Start Simulation"
              API call
            </li>
            <li>
              <strong>Phase Control:</strong> Use Next/Previous/Set buttons to
              cycle through predefined phases
            </li>
            <li>
              <strong>Direct State Control:</strong> Set exact traffic light
              states using SUMO notation:
              <ul>
                <li>
                  <code>G</code> = Green, <code>r</code> = Red, <code>y</code> =
                  Yellow
                </li>
                <li>
                  <code>GrGr</code> = North/South Green, East/West Red
                </li>
                <li>
                  <code>rGrG</code> = North Red, South Green, East Red, West
                  Green
                </li>
              </ul>
            </li>
            <li>
              Watch your SUMO simulation window for immediate visual changes
            </li>
            <li>
              Check browser console and backend logs for detailed operation logs
            </li>
          </ul>
          <p>
            <strong>Note:</strong>{" "}
            {tlsData
              ? `${tlsData.friendlyCount} friendly TLS names are mapped to real SUMO traffic light IDs from your network.`
              : "TLS mapping is loading..."}
          </p>
          {tlsData && tlsData.totalCount > tlsData.friendlyCount && (
            <p>
              <strong>Info:</strong> Your network has {tlsData.totalCount} total
              traffic lights, {tlsData.friendlyCount} have friendly names.
            </p>
          )}
        </div>
      </div>
